  ...
  useFrame((_, delta) => {
    if (mode === 'walkthrough') {
      ...
      const groundedHeight =
        (runtimeBounds ? runtimeBounds.min[1] : 0) + EYE_HEIGHT
    ...
    if (hotspot) {
      ...
      destination.copy(modelFrame ? modelFrame.position : OVERVIEW_POSITION)
      target.copy(modelFrame ? modelFrame.target : OVERVIEW_TARGET)
    }
